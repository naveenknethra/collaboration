	   ...
	   public ResponseEntity<User> register(@RequestBody User user ,  HttpServletRequest request)
	   {
		...
		/*   
		  MultipartFile file=user.getImage();
			...
			path=(Path) Paths.get(rootDirectory + "\\resources\\image\\"+user.getId()+".jpg");
			
			
			if(file!=null && !file.isEmpty()){
				try{
					file.transferTo(new File(path.toString()));
					System.out.println("User "+user.getId()+" Image is getting Uploaded.....");
				}catch(Exception e)
				{
					e.printStackTrace();
			...
			System.out.println("User "+user.getId()+" Image Uploaded.....");
